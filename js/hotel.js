...
let resort = {
    ...
    "endDate": "2019-12-31",
    "shortDescription": "The resort of Les Boucaniers is located on the laid-back beach-covered south coast of the island, and is perfectly placed for Martinique holidays that are both relaxing and awe-inspiring.",
    ...
    "reviews": [
        {
            "description": "Spectacular property. Excellent staff! Can’t wait to come back! Counting down the days till my next holiday there",
            "author": "Oscar Vazquez"
        },
        {
            "description": "This is one of the best islands in the Caribbean. The resort has a beautiful beach and the water sports are great.",
            "author": "Alyssa Mincer"
        },
        {
            "description": "Amazing stay at Les Boucaniers. Incredible GO team, beautiful village, spectacular food.",
            "author": "Kera Pezzuti"
        }
    ]
};
...
$(document).ready(function () {
    ...
    $.getJSON('./resources/fake.json', function (data) {
        ...
        $('.male-author-1 .author .date').text(new Date(+(new Date()) - Math.floor(Math.random() * 10000000000)));
        $('.male-author-1 .author .rounded-circle').prop('src', `img/user-picture/male-${num}.svg`);
        ...
        $('.female-author-1 .author .date').text(new Date(+(new Date()) - Math.floor(Math.random() * 10000000000)));
        $('.female-author-1 .author .rounded-circle').prop('src', `img/user-picture/female-${num}.svg`);
        ...
        $('.female-author-2 .author .date').text(new Date(+(new Date()) - Math.floor(Math.random() * 10000000000)));
        $('.female-author-2 .author .rounded-circle').prop('src', `img/user-picture/female-${num - 1}.svg`);

        $('#location-text').text(`${resort.name}, ${resort.location},  ${resort.destination}`);
